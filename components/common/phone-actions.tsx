...
/** Normalize to international digits for tel:/wa.me (default India +91 for 10-digit). */
export function getWhatsAppDigits(mobile?: string | null): string | null {
...
export function PhoneActions({
  ...
  const digits = getWhatsAppDigits(mobile)
  const display = mobile?.trim() || "N/A"

  if (!digits) {
    return showNumber ? (
      <span className={cn("text-xs text-muted-foreground", numberClassName)}>{display}</span>
    ) : null
  }
  ...
  return (
    <div className={cn("flex items-center gap-1.5 min-w-0", className)}>
      {showNumber && (
        <span className={cn("text-xs text-muted-foreground whitespace-nowrap", numberClassName)}>
          {display}
        </span>
      )}
      <a
